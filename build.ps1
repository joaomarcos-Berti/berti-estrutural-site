...
$CSS = @'
  ...
  *{ box-sizing:border-box; }
  ...
  .nav__links a:not(.nav__cta){ color:rgba(255,255,255,0.82); text-decoration:none; transition:color 180ms ease; }
...
try{if(localStorage.getItem('berti_cookie_consent')==='accepted'){gtag('consent','update',{ad_storage:'granted',analytics_storage:'granted',ad_user_data:'granted',ad_personalization:'granted'});}}catch(e){}
gtag('js',new Date());gtag('config','G-B5M982KVN0');</script>
<title>{{TITLE}}</title>
...
{{MAIN}}
{{FOOTER}}
<div id="ck-consent" role="dialog" aria-label="Aviso de cookies" hidden>
<p>Usamos cookies para medir o tráfego do site (Google Analytics) e melhorar sua experiência. Você pode aceitar ou recusar a medição.</p>
<div class="ck-acts"><button type="button" id="ck-no">Recusar</button><button type="button" id="ck-yes">Aceitar</button></div>
</div>
<style>
#ck-consent[hidden]{display:none}
#ck-consent{position:fixed;left:16px;right:16px;bottom:16px;z-index:9999;max-width:760px;margin:0 auto;background:#0a0f16;color:#e8eef4;border:1px solid rgba(71,182,241,.25);border-radius:14px;padding:16px 18px;display:flex;gap:14px;align-items:center;justify-content:space-between;flex-wrap:wrap;box-shadow:0 10px 40px rgba(0,0,0,.45);font:14px/1.5 system-ui,-apple-system,"Segoe UI",Roboto,sans-serif}
#ck-consent p{margin:0;flex:1 1 320px}
#ck-consent .ck-acts{display:flex;gap:10px;flex:0 0 auto}
#ck-consent button{cursor:pointer;border-radius:9px;padding:9px 18px;font-weight:600;font-size:14px;border:1px solid transparent;transition:background .15s,border-color .15s}
#ck-no{background:transparent;color:#cdd6df;border-color:rgba(255,255,255,.18)}
#ck-no:hover{border-color:rgba(255,255,255,.45)}
#ck-yes{background:#077fbf;color:#fff}
#ck-yes:hover{background:#0a8fd6}
@media(max-width:560px){#ck-consent{flex-direction:column;align-items:stretch}#ck-consent .ck-acts{justify-content:flex-end}}
</style>
<script>(function(){var K='berti_cookie_consent',el=document.getElementById('ck-consent');if(!el)return;try{if(localStorage.getItem(K)){return;}}catch(e){}el.hidden=false;function set(v){try{localStorage.setItem(K,v);}catch(e){}if(v==='accepted'&&typeof gtag==='function'){gtag('consent','update',{ad_storage:'granted',analytics_storage:'granted',ad_user_data:'granted',ad_personalization:'granted'});}el.hidden=true;}document.getElementById('ck-yes').addEventListener('click',function(){set('accepted');});document.getElementById('ck-no').addEventListener('click',function(){set('rejected');});})();</script>
<script>(function(){function lead(m){if(typeof gtag==='function'){gtag('event','generate_lead',{method:m});}}document.addEventListener('click',function(e){var a=e.target.closest?e.target.closest('a'):null;if(!a)return;var h=(a.getAttribute('href')||'').toLowerCase();if(h.indexOf('wa.me')>-1||h.indexOf('api.whatsapp')>-1||h.indexOf('whatsapp.com')>-1){lead('whatsapp');}else if(h.indexOf('tel:')===0){lead('phone');}},true);document.addEventListener('submit',function(e){if(e.target&&e.target.tagName==='FORM'){lead('form');}},true);})();</script>
</body>
</html>
'@
  ...
  <section class="cbody"><div class="in">
    ...
    <aside class="cficha">
      ...
      <div><div class="ccap">Redes sociais</div><div class="csoc">
        <a href="https://www.instagram.com/bertiestrutural/" target="_blank" rel="noopener" title="Instagram"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.16c3.2 0 3.58.01 4.85.07 1.17.05 1.8.25 2.23.41.56.22.96.48 1.38.9.42.42.68.82.9 1.38.16.42.36 1.06.41 2.23.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.05 1.17-.25 1.8-.41 2.23a3.7 3.7 0 0 1-.9 1.38c-.42.42-.82.68-1.38.9-.42.16-1.06.36-2.23.41-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-1.17-.05-1.8-.25-2.23-.41a3.7 3.7 0 0 1-1.38-.9 3.7 3.7 0 0 1-.9-1.38c-.16-.42-.36-1.06-.41-2.23C2.17 15.58 2.16 15.2 2.16 12s.01-3.58.07-4.85c.05-1.17.25-1.8.41-2.23.22-.56.48-.96.9-1.38.42-.42.82-.68 1.38-.9.42-.16 1.06-.36 2.23-.41C8.42 2.17 8.8 2.16 12 2.16z"/><path d="M12 5.84a6.16 6.16 0 1 0 0 12.32 6.16 6.16 0 0 0 0-12.32zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.4-11.85a1.44 1.44 0 1 0 0 2.88 1.44 1.44 0 0 0 0-2.88z"/></svg></a>
...
(function(){var fr=document.querySelectorAll('.eh__f'),dt=document.querySelectorAll('#eh_dots button'),i=0;
 ...
 function par(){ var mob=window.innerWidth<=900, vh=window.innerHeight;
   steps.forEach(function(st,idx){ var m=st.querySelector('.pmedia'), t=st.querySelector('.ptext'); if(!m||!t)return;
     if(mob){ m.style.transform=''; m.style.opacity=''; t.style.transform=''; t.style.opacity=''; return; }
     var r=st.getBoundingClientRect(); var n=Math.max(-1.1,Math.min(1.1,(r.top+r.height/2-vh/2)/(vh*0.85))); var sd=(idx%2===1)?-1:1;
     m.style.transform='translateX('+(n*26*sd)+'%)'; m.style.opacity=String(1-Math.min(1,Math.abs(n))*0.35);
...
<style>
  ...
  .e3stage model-viewer{ position:absolute; inset:0; width:100%; height:100%; z-index:1; background:transparent; --poster-color:transparent; --progress-bar-color:transparent; --progress-bar-height:0px; }
  .e3stage model-viewer::part(default-progress-bar){ display:none !important; }
  ...
  .hb__head{ display:flex; align-items:flex-end; justify-content:space-between; gap:16px; margin-bottom:36px; max-width:1440px; margin-left:auto; margin-right:auto; }
  .hb__grid{ display:grid; grid-template-columns:repeat(3,1fr); gap:24px; max-width:1440px; margin:0 auto; }
...
foreach($o in $mapObras){
  ...
  [void]$mapData.Append("{t:`"$(JsonStr $o.title)`",c:`"/$($o.cover)`",a:`"$(JsonStr $addr)`",lat:$(Num $o.lat),lng:$(Num $o.lng),s:`"$($o._slug)`"},")
  $mi++
}
...
(function(){
  ...
  function initMap(){
    ...
    for(var i=0;i<MAP.length;i++){
      ...
      m.bindPopup('<a href="/obras/'+o.s+'" style="width:220px;display:block;font-family:Open Sans,sans-serif;text-decoration:none;color:inherit;cursor:pointer"><img src="'+o.c+'" alt="'+o.t+'" style="width:100%;height:115px;object-fit:cover;display:block"/><div style="padding:11px 13px"><div style="font-family:Barlow Condensed,sans-serif;font-weight:800;font-size:17px;text-transform:uppercase;color:#061922;line-height:1">'+o.t+'</div><div style="margin-top:5px;font-size:12.5px;color:#5b6b75">'+o.a+'</div><div style="margin-top:8px;font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:#077fbf">Ver detalhes da obra &rarr;</div></div></a>',{minWidth:220});
      mk.push(m);
    }
